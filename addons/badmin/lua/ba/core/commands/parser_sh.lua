...
function ba.cmd.AutoComplete(cmd, str)
	...
	if args[1] and ba.cmd.Exists(args[1]) then
		...
		for k, v in ipairs(cmd_obj:GetArgs()) do
			...
			local tbl = ba.cmd.Param(v.Param).AutoComplete and ba.cmd.Param(v.Param):AutoComplete(args[k + 1], {Key = v.Key, Pos = k, Args = args}) or {}
...
local RunCommand = (SERVER) and ba.cmd.ConCommand or function(p, c, a, str) p:ConCommand('_urf ' .. str) end
concommand.Add('urf', RunCommand, ba.cmd.AutoComplete)
concommand.Add('urfadmin', RunCommand, ba.cmd.AutoComplete)
...
local function PlayerAutoComplete(self, arg, opts)
	...
	if (arg ~= nil) and ba.IsSteamID(arg) then 
		...
		for _, pl in ipairs(player.GetAll()) do
			if (arg ~= nil) and string.find(string.utfnotutflower(pl:Name()), string.utfnotutflower(arg) ) then
				ret[#ret + 1] = pl:Name()
			elseif (arg == nil) then
				ret[#ret + 1] = pl:Name()
			end
...
ba.cmd.Param('player_steamid')
	:Parse(function(self, pl, cmd, arg, opts)
		local EIA = ba.cmd.Get(cmd).EqualImmunityAccess or false
		local result = ba.FindPlayer(arg)
		if (result ~= nil) and not ba.ranks.CanTarget(pl, result, EIA) then
