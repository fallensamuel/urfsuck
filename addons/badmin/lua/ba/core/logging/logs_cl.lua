...
local function LayoutLogs(cont, data)
	local s = ui.Create('DListView', function(self, p)
		...
		self:AddColumn('Time (UTC)'):SetFixedWidth(115)
		self:AddColumn('Data')
...
local function PlayerEvents()
	...
	local lbl = ui.Create('DLabel', function(self, p)
		self:SetPos(5, 35)
		self:SetText('Enter a Player/SteamID to search')
		self:SetFont('ui.20')
	...
	local srch = ui.Create('DButton', function(self, p)
		...
		self.DoClick = function(self)
			RunConsoleCommand('urf', 'playerevents', txt:GetValue())
			p:Close()
		end
	end, fr)
end
...
function OpenMenu(data)
	...
	fr.tabs:AddButton('Playerevents', function()
		fr:Close()
		PlayerEvents()
	end)
	...
	function fr:AddData(name, data)
		...
		local lbl = ba.ui.Label('Search:', cont, {
			font = 'ui.22',
			color = ui.col.Close
