...
function ba.ranks.CanTarget(pl, targ, equalImmunityAccess)
	equalImmunityAccess = equalImmunityAccess or false
	if (not ba.IsPlayer(pl)) or pl:IsRoot() or ((ba.IsSuperAdmin(pl)) and not (targ:IsRoot() or ba.IsSuperAdmin(targ) and pl:GetImmunity() < targ:GetImmunity())) then return true end
	if (equalImmunityAccess) then return (pl:GetImmunity() >= targ:GetImmunity()) end
...
function rank_mt:SetImmunityGiveRank(amt)
	self.ImmunityGiveRank = amt
	return self
end
...
function rank_mt:GetImmunityGiveRank()
	return self.ImmunityGiveRank
end
...
function PLAYER:GetImmunityGiveRank()
	return self:GetRankTable():GetImmunityGiveRank()
end
...
function PLAYER:IsVIP()
	return rp.EventIsRunning('vip') or hook.Call('PlayerVIPCheck', GAMEMODE, self) == true or self:GetRankTable():IsVIP()
end
...
function ba.IsSuperAdmin(pl)
	if (hook.Call('PlayerAdminCheck', GAMEMODE, pl) == false) then return false end
	return pl:GetRankTable():IsSuperAdmin()
end
