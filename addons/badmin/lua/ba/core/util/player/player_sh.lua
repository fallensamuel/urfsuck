...
function ba.FindPlayer(info) 
	...
	for k, pl in ipairs(player.GetAll()) do
		...
		if string.find(string.utfnotutflower(pl:Name()), string.utfnotutflower(tostring(info)), 1, true) ~= nil then
			return pl
		end
	end
	return nil
...
function ba.InfoTo64(info)
	return ((ba.IsPlayer(info) and IsValid(info)) and info:SteamID64() or util.SteamIDTo64(info))
end
