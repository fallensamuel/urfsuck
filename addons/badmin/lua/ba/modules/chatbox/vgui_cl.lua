...
cvar.Register'dont_show_chat':SetDefault(false):AddMetadata('State', 'RPMenu'):AddMetadata('Menu', 'Выключить чат')
...
function LABEL:Paint(w, h)
    if (SysTime() > self.Expire) and (CHATBOX_UI and !CHATBOX_UI._Open) then return true end
    
	if cvar_Get('dont_show_chat') and CHATBOX_UI and !CHATBOX_UI._Open then return true end
    local fin = math.Clamp((SysTime() - (self.Expire - 15)) / .25, 0, 1)
    if (!CHATBOX_UI._Open and fin == 1) then
...
function PANEL:Init()
    ...
    self.txtEntry.OnKeyCodeTyped = function(s, c)
        ...
        elseif (c == KEY_ENTER) then
            ...
            local text = utf8_sub(s:GetValue(), 1, 160)
            if utf8.len(text) > 126 then
                rp.PlayerSay(text, self._Team)
            else
                RunConsoleCommand("say" .. ((self._Team and "_team") or ""), text)
            end
    ...
    self.txtEntry.OnTextChanged = function(s)
        ...
        if (s:AllowInput()) then
            s:SetValue(string.sub(s:GetValue(), 1, 160))
            s:SetCaretPos(160)
        end
...
function PANEL:PerformLayout()
    ...
    self:SetPos(10, ScrH() - self:GetTall() - 10 - 120 - 86) 
...
if IsValid(CHATBOX_UI) then CHATBOX_UI:Remove() CHATBOX_UI = ba.CreateChatBox() end