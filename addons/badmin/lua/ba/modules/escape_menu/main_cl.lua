...
cvar.Create('EnableCustomEscape', true)

ba.ui.AddCVar('EnableCustomEscape', 'Enable escape menu')
...
hook.Add('PreRender', 'esc menu', function()
	if input.IsKeyDown(KEY_ESCAPE) and gui.IsGameUIVisible() then
		if IsValid(panel) then
			gui.HideGameUI()
			panel:Remove()
		elseif (cvar.GetValue('EnableCustomEscape') == true) then
			gui.HideGameUI()
			ba.esc.openMenu()
		end
	end
...
ba.esc.openMenu = function()
	gui.EnableScreenClicker(true)

	panel = ui.Create('ui_panel')
	...
	local header = ui.Create('ui_panel', panel)
	...
	local btnClose = ui.Create('DButton', header)
	...
	local avatarImage = ui.Create('esc_menu_avatarimage', panel)
	...
	local body = ui.Create('ui_panel', panel)
	...
	local function addButton(parent, text, action)
		local btn = ui.Create('DButton', parent)
	...
	local function addSection(parent, title)
		local panel = ui.Create('ui_panel', parent)
	...
	addServerButton('ZOMBIE SURVIVAL', 'zs.superiorservers.co')


	local webList = addSection(body, 'WEB LINKS')
	addAltText(addButton(webList, 'FORUMS', function()
		gui.OpenURL('http://superiorservers.co/')
	end), 'CLICK TO OPEN')
	addAltText(addButton(webList, 'BAN LIST', function()
		gui.OpenURL('http://superiorservers.co/portal/bans.php')
	end), 'CLICK TO OPEN')
	addAltText(addButton(webList, 'STAFF LIST', function()
		gui.OpenURL('http://superiorservers.co/portal/staff.php')
	end), 'CLICK TO OPEN')
