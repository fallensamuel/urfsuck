ba.AddTerm('AbuseAdminUnjailedPlayer', '# выпустил из Алькатраса #.')
ba.AddTerm('AbuseAdminJailedPlayer', '# посадил в Алькатрас #.')
ba.AddTerm('AbusePlayerNotAlive', '# мёртв!')
ba.AddTerm('AbuseAdminSlainPlayer', '# был убит #.')
ba.AddTerm('AdminCloackEnable', '# стал невидимым.')
ba.AddTerm('AdminCloackDisable', '# вышел из невидимости.')
...
ba.cmd.Create('Cloack', function(pl, args)
	if pl:GetNoDraw() then
		ba.notify_staff(ba.Term('AdminCloackDisable'), pl)
		pl:SetNoDraw(false)
		pl:SetMaterial("")
		pl:SetColor(Color(255, 255, 255))
	else
		ba.notify_staff(ba.Term('AdminCloackEnable'), pl)
		pl:SetNoDraw(true)
