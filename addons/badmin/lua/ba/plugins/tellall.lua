ba.AddTerm('AdminSentStaffMessage', '# Написал оповещение: #')
...
if (CLIENT) then
	...
	function PANEL:Init()
		...
		self.Title:SetText((translates and translates.Get('Администрация') or 'Администрация') .. '!')
	end
	...
	function PANEL:Think()
		if(CurTime() > self.RunOut) then -- Something gone wrong, if that happened
			self:Remove()
			return
		end
