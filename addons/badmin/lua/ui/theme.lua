...
function SKIN:PaintUISlider(self, w, h)
	...
	val = tonumber(self:GetValue()) or 0
	draw.Box(1, 1, val * w - val * 16, h - 2, color_sup)
end
...
function SKIN:PaintListViewLine(self, w, h) -- todo, just make a new control and never use this
	if self.m_bAlt then
		draw.Box(0, 0, w, h, (self:IsSelected() or self:IsHovered()) and color_sup or color_white)
	else
		draw.Box(0, 0, w, h, (self:IsSelected() or self:IsHovered()) and color_sup or color_offwhite)
	end
