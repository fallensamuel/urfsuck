...
local cvar_staging = {}
local function load()
	if file.IsDir('cvar', 'DATA') then
		local files, _ = file.Find('cvar/*.dat', 'DATA')
		for k, v in ipairs(files) do
			local c = setmetatable(decode(file.Read('cvar/' .. v, 'DATA')), cvar_mt)
			if c:GetMetadata('Type') and c:GetMetadata('Type') ~= type(c:GetValue()) then c:Reset() end
...
function cvar_mt:SetValue(value)
	hook.Run('cvar.' ..  self.Name, nil, self.Value, value)
	self.Value = value
	self:Save()
	return self
end
