...
function ENT:Think()
	...
	if self.RunNext < curtime and LocalPlayer():EyePos():DistToSqr(self:GetPos()) < 1000000 then
		...
		if not self.CustomWheels then
			for i = 1, table.Count( self.pp_data ) do
				...
				if IsValid( Wheel ) then
					local addPos = Wheel.GetDamaged and Wheel:GetDamaged() and self.pp_data[i].dradius or 0
...
function ENT:GetEnginePos()
	...
	if self.EnginePos then
		return self:LocalToWorld( self.EnginePos )
	end
	
	local vehiclelist = list.Get( "simfphys_vehicles" )[ self:GetSpawn_List() ]
	if vehiclelist then
		self.EnginePos = vehiclelist.Members.EnginePos or false
		return self.EnginePos and self:LocalToWorld( self.EnginePos ) or self:GetPos()
	end
...
function ENT:ManageEffects( Active, fThrottle, LimitRPM )
	if self.EffectNext and self.EffectNext < CurTime() then return end
	self:DamageEffects()
	...
	local Scale = fThrottle * math.min(self:GetRPM() / LimitRPM, 1)
	Scale = Scale * Scale
	
	for i, tab in pairs(self.ExhaustPositions) do
		if tab.OnBodyGroups then
			if self:BodyGroupIsValid(tab.OnBodyGroups) then
				local effectdata = EffectData()
					effectdata:SetOrigin(tab.pos)
					effectdata:SetAngles(tab.ang)
