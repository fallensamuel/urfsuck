...
function ENT:Think()
	...
	--------------------------------
	self.NextSeatFix = self.NextSeatFix or 0
	
	if self.NextSeatFix < Time then
		self.NextSeatFix = Time + 0.5
		
		if IsValid(self.DriverSeat) and IsValid(self.DriverSeat:GetDriver()) then
			self.DriverSeat:GetDriver():SetPos(Vector())
		end
		
		if self.pSeat then 
			for i = 1, table.Count(self.pSeat) do 
				if IsValid(self.pSeat[i]) and IsValid(self.pSeat[i]:GetDriver()) then
					self.pSeat[i]:GetDriver():SetPos(Vector())
				end
			end
		end
	end
