...
SWEP.Category				= "Half-Life Alyx SH"
...
if SERVER then
	...
	net.Receive( "simfphys_gasspill", function( length, ply )
		...
		if ply:GetPos():Distance(Pos) > MaxDistance * 2 then
			simfphys:exploited(ply, 'simfphys_gasspill')
			return
		end
...
function SWEP:Think()
	if CLIENT then
		...
		if InRange and (not HIT or (IsValid( ent ) and simfphys.IsCar( ent ) and ent:GetFuel() >= ent:GetMaxFuel()) or (IsValid( ent ) and simfphys.IsCar( ent ) and not self:IsAimingAt( ent:GetFuelPos() ) ) ) then
...
function SWEP:Holster()
	if IsValid(self.pViewModel) then
		SafeRemoveEntity(self.pViewModel)
	end
