...
if ( CLIENT ) then
	SWEP.DrawAmmo			= true
	SWEP.PrintName			= "Электрическая атака"
	SWEP.Author				= "Jvs"
...
SWEP.AmmoPerUse				= 15		 --we use ar2 altfire ammo,don't exagerate here	
...
SWEP.BeamDamage				= 250	//25 is done by one zap attack,since vortigaunt has two claws,50 dmg,
...
SWEP.Primary.DefaultClip	= 50		//give the poor user 25 combine balls to have fun with this wepon
...
function SWEP:Precache()
	...
	util.PrecacheModel(self.ViewModel)					--he... come on,that's obvious
end
...
function SWEP:Think()
	...
	if self.Healing and self.HealTime < CurTime() then
	    if Owner:GetAmmoCount(self.Primary.Ammo) < self.AmmoPerUse or Owner:Armor() >= Owner:GetMaxArmor() then
	        self.Weapon:EmitSound(self.Deny)
	        self.Weapon:SetNextPrimaryFire(CurTime() + SoundDuration(self.Deny))
	        self.Weapon:SetNextSecondaryFire(CurTime() + SoundDuration(self.Deny))
	    ...
	    Owner:RemoveAmmo(self.AmmoPerUse, self.Primary.Ammo)
	...
	if SERVER && self.NextAmmo < CurTime() && Owner:GetAmmoCount("AR2AltFire") < 99 then
		Owner:GiveAmmo(1, "AR2AltFire", true)
		self.NextAmmo = CurTime() + .3
	end
end
...
function SWEP:GiveArmor()
	...
	local max = self.Owner:GetMaxArmor()
	if plarm >= max then return end

	if plarm <= (max - arm) then
	    self.Owner:SetArmor(plarm + arm)
	else
	    self.Owner:SetArmor(max)
	end

	self.Owner:SetHealth(
		math.min(self.Owner:Health() + math.random(2, 8), self.Owner:GetMaxHealth())
	)
end
...
function SWEP:SecondaryAttack()
	if self.Charging || self.Healing || self.Owner:Armor() >= self.Owner:GetMaxArmor() then return end
		if self.Owner:GetAmmoCount(self.Primary.Ammo)<self.AmmoPerUse  then 
		self.Weapon:EmitSound(self.Deny)
		self.Weapon:SetNextPrimaryFire(CurTime()+SoundDuration(self.Deny))
		self.Weapon:SetNextSecondaryFire(CurTime()+SoundDuration(self.Deny))
