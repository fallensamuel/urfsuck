-- "gamemodes\\darkrp\\entities\\entities\\server_changer_fallout.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
ENT.Base = 'base_ai'
ENT.Type = 'ai'
ENT.PrintName = 'Server Changer Fallout'
...
if SERVER then
	util.AddNetworkString('ServerChange3')

	function ENT:Initialize()
		self:SetModel('models/models/frix/x01/xo1_powerarmor_npc.mdl')
	...
	function ENT:Draw()
		...
		if lastCheck < CurTime() then
			...
			http.Fetch(info.handler..info.AltServerIP3, function(body)
		...
		draw.SimpleTextOutlined('Отправиться в '..info.name3, '3d2d', 0, -450, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
		cam.End3D2D()
		ang:RotateAroundAxis(ang:Right(), 180)
		cam.Start3D2D(pos, ang, 0.070)
		draw.SimpleTextOutlined(data, '3d2d', 0, -450, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
		draw.SimpleTextOutlined('Отправиться на '..info.name3, '3d2d', 0, -450, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
		cam.End3D2D()
	end

	net.Receive('ServerChange3', function(len, ply)
		local title = 'Отправиться на '..info.name3
		local text = info.desc3

		function cback(result)
			if result then LocalPlayer():ConCommand('connect '..info.AltServerIP3) end
		end
