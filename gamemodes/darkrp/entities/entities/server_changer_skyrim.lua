-- "gamemodes\\darkrp\\entities\\entities\\server_changer_skyrim.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
ENT.Base = 'base_ai'
ENT.Type = 'ai'
ENT.PrintName = 'Server Changer Skyrim'
...
if SERVER then
	util.AddNetworkString('ServerChange4')

	function ENT:Initialize()
		self:SetModel('models/player/guard_solitude_npc.mdl')
	...
	function ENT:Draw()
		...
		if lastCheck < CurTime() then
			...
			http.Fetch(info.handler..info.AltServerIP4, function(body)
				body = util.JSONToTable(body)
				if !body.error then
					data = 'На нем играют: '..body.result
				end
			end)
		end
		...
		cam.Start3D2D(pos + ang:Up() * 9, ang, 0.070)
		draw.SimpleTextOutlined(data, '3d2d', 0, -450, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
		draw.SimpleTextOutlined('Отправиться в '..info.name4, '3d2d', 0, -450, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
		cam.End3D2D()
		ang:RotateAroundAxis(ang:Right(), 180)
		cam.Start3D2D(pos + ang:Up() * 9, ang, 0.070)
		draw.SimpleTextOutlined(data, '3d2d', 0, -450, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
		draw.SimpleTextOutlined('Отправиться на '..info.name4, '3d2d', 0, -450, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
		cam.End3D2D()
	end

	net.Receive('ServerChange4', function(len, ply)
		local title = 'Отправиться на '..info.name4
		local text = info.desc4

		function cback(result)
			if result then LocalPlayer():ConCommand('connect '..info.AltServerIP4) end
		end
