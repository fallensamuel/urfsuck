-- "gamemodes\\darkrp\\entities\\weapons\\base_sweps_detector.lua"
...
SWEP.Primary.Ammo         		= "detector_bolts"
...
SWEP.Arts = {}
SWEP.SearchRadius = 500
SWEP.BeepSound = Sound("stalkerdetectors/echo.wav")

local throw_sound = Sound("weapons/slam/throw.wav")
	...
	if (self.UseDel < CurTime() and self:Ammo1() > 0) then
		...
		if (SERVER) then
			timer.Simple( 0.9, function()
				...
				self:EmitSound( throw_sound, 100, 100 )
...
function SWEP:Initialize()
	self.SearchRadiusSq = self.SearchRadius ^ 2
	...
	**************************/
	...
	function SWEP:Think()
		if self.LastCheck < CurTime() then
			...
			for k, v in pairs(ents.FindInSphere(self.Owner:GetPos(), self.SearchRadius)) do
				if rp.cfg.Artefacts[v:GetNWString("uniqueID")] then
					self.Arts[#self.Arts + 1] = v
				end
			end

			local min_dist, dist = nil, 0
			for k, v in pairs(self.Arts) do
				min_dist = v:GetPos():DistToSqr(self.Owner:GetPos())
				if !min_dist || dist < min_dist then
					min_dist = dist
					self.ClosestArt = v
				end
			end
		end
		if IsValid(self.ClosestArt) then
			local dist = self.ClosestArt:GetPos():DistToSqr(self.Owner:GetPos())
			if self.LastBeep + dist/self.SearchRadiusSq - CurTime() <= 0 then
