-- "gamemodes\\darkrp\\entities\\weapons\\health_kit_best_d2.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
AddCSLuaFile()
SWEP.Base = 'weapon_rp_base'

if CLIENT then
	SWEP.PrintName = "Научная Аптечка 2"
	SWEP.Slot = 5
	SWEP.SlotPos = 0
...
--SWEP.MaxRestore = 1000

SWEP.ViewModel = Model("models/flaymi/anomaly/dynamics/devices/dev_aptechka_mid.mdl")
SWEP.UseHands = false
SWEP.HoldType = "slam"
...
SWEP.WorldModel = Model("models/flaymi/anomaly/dynamics/devices/dev_aptechka_mid.mdl")
if CLIENT then
...
function SWEP:PrimaryAttack()
	...
	local maxhealth = (IsValid(ent) and ent:IsPlayer()) and 250
	if not isnumber(health) or health >= maxhealth or (self.Owner:GetPos():Distance(ent:GetPos()) > self.HitDistance) then return end
	--self.CurRestore = self.CurRestore - (health > maxhealth - 5 and maxhealth - health or 5)
	ent:SetHealth(health > maxhealth - 15 and maxhealth or health + 15)
	self.Owner:EmitSound(self.Primary.Sound, 125, health)
	
	--self:TakePrimaryAmmo( health > maxhealth - 25 and maxhealth - health or 25 )
...
function SWEP:SecondaryAttack()
	...
	local maxhealth = SERVER and 250
	if CLIENT or health >= maxhealth then return end
	--self.CurRestore = self.CurRestore - (health > maxhealth - 5 and maxhealth - health or 5)
	self.Owner:SetHealth(health > maxhealth - 15 and maxhealth or health + 15)
	self.Owner:EmitSound(self.Primary.Sound, 125, health)
