-- "gamemodes\\darkrp\\entities\\weapons\\health_kit_normal.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
AddCSLuaFile()
SWEP.Base = 'weapon_rp_base'

if CLIENT then
	SWEP.PrintName = "Армейская Аптечка"
	SWEP.Slot = 5
	SWEP.SlotPos = 0
...
SWEP.ViewModel = Model("models/flaymi/anomaly/dynamics/devices/dev_aptechka_high.mdl")
SWEP.UseHands = false
SWEP.HoldType = "slam"
...
SWEP.WorldModel = Model("models/flaymi/anomaly/dynamics/devices/dev_aptechka_high.mdl")
if CLIENT then
...
function SWEP:PrimaryAttack()
	...
	local maxhealth = (IsValid(ent) and ent:IsPlayer()) and 200 --math.min(ent:GetMaxHealth(), 200)
	if not isnumber(health) or health >= maxhealth or (self.Owner:GetPos():Distance(ent:GetPos()) > self.HitDistance) then return end
	--self.CurRestore = self.CurRestore - (health > maxhealth - 3 and maxhealth - health or 3)
	ent:SetHealth(health > maxhealth - 10 and maxhealth or health + 10)
	self.Owner:EmitSound(self.Primary.Sound, 125, health)
	
	--self:TakePrimaryAmmo( health > maxhealth - 15 and maxhealth - health or 15 )
...
function SWEP:SecondaryAttack()
	...
	local maxhealth = SERVER and 200 --math.min(self.Owner:GetMaxHealth(), rp.cfg.MaxHealth)
	if CLIENT or health >= maxhealth then return end
	--self.CurRestore = self.CurRestore - (health > maxhealth - 3 and maxhealth - health or 3)
	self.Owner:SetHealth(health > maxhealth - 10 and maxhealth or health + 10)
	self.Owner:EmitSound(self.Primary.Sound, 125, health)
