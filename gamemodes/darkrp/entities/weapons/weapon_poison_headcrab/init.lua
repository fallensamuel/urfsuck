...
function SWEP:Deploy()
	...
	self.Owner:SetModel('models/headcrabblack.mdl')
	self:IdleSounds()
...
function SWEP:RemoveTimers()
	...
	timer.Remove('hitHeadcrabPoison.' .. self:EntIndex())
...
function SWEP:PrimaryAttack()
	...
	timer.Simple(0.1, function()
		timer.Create('hitHeadcrabPoison.' .. self:EntIndex(), 0.05, 0, function() 
			...
			timer.Create("headcrabDigIdle", 0.5, 0, function() 
				...
				if self.IsUnderGround then
					...
					if self.Owner:Health() < self.Owner:GetMaxHealth()  then
						...
						self.Owner:SetHealth(self.Owner:Health() + self.jRegenPerSecond / 2)
