...
function DRUG:StartHighServer(pl)
	...
	pl:SetWalkSpeed(pl.StunstickOldRunSpeed * 0.9);
	pl:SetRunSpeed(pl.StunstickOldWalkSpeed * 0.9);
end
...
function DRUG:AddStack(pl, stacks)
	pl:SetWalkSpeed(pl:GetRunSpeed() * 0.9);
	pl:SetRunSpeed(pl:GetRunSpeed() * 0.9);
end
...
local RoarSound = {Sound('watchman_howl_distant_1.wav'), Sound('watchman_howl_distant_3.wav')}

local anim_zombie = {
	[ACT_MP_STAND_IDLE] = ACT_HL2MP_WALK_ZOMBIE_06,
...
function SWEP:PrimaryAttack( right )
	...
	if SERVER then
		self.Owner:forceSequence("attack")
	else
