...
function ENT:UseFunc(ply)
	...
	if old_hp == ply:GetMaxHealth() then
		rp.Notify(ply, NOTIFY_RED, rp.Term('SPP_MaxHealth'))
		return
	end
	...
	if new_hp > old_hp then
		...
		rp.Notify(ply, NOTIFY_GREEN, rp.Term('PlayerTookSomeFromPrinter'), translates and translates.Get( "здоровье на %i", new_hp - old_hp ) or ("здоровье на `"..(new_hp-old_hp).."`"))
	end
end