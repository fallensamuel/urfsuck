...
local function Finalize(ent, tab, owner)
	...
	if !util.IsValidModel(tab.Model) then ent:Remove() return end
...
function SWEP:PrimaryAttack()
	...
	if (not ph:IsValid()) then return end

	if (not wl[ent:GetClass()]) then
		self.Owner:ChatPrint("Вы не можете положить " .. ent:GetClass() .. " в вашу сумку.")
	...
	if (table.Count(p) >= Limit) then
		self.Owner:ChatPrint("Ваша сумка заполнена")
...
net.Receive("Pocket.AdminDelete", function(len, pl)
	...
	local item = inv[id]
	local itemName = (item.contents and rp.shipments[item.contents].name .. ' (Шт: ' .. item.count .. ')') or rp.inv.Wl[item.Class]
	targ:GetInv()[id] = nil
	targ:SaveInv()
	net.Start("Pocket.RemoveItem")
	net.WriteUInt(id, 32)
