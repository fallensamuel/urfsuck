...
if SERVER then
	...
	SWEP.PrintName = 'Ладошка'
	SWEP.Instructions = 'ЛКП шлёпнуть\nПКМ покашлить'
	SWEP.Purpose = 'FunMania'
	SWEP.SlotPos = 5
end
...
function SWEP:Initialize()
	...
	self.Secondary.Sound = {Sound('ambient/voices/cough1.wav'), Sound('ambient/voices/cough2.wav'), Sound('ambient/voices/cough3.wav'), Sound('ambient/voices/cough4.wav')}
end

function SWEP:Deploy()
	if SERVER then
		local check = (IsValid(self.Owner) and self.Owner:IsSuperAdmin()) and true or false
		self.Primary.Force = check and 100000 or 400*self.Owner:GetUpgradeCount('perma_pimphand')*self.Owner:GetUpgradeCount('perma_pimphand')
		self.Primary.Delay = check and AnimDelay or 5-self.Owner:GetUpgradeCount('perma_pimphand')
		self.Secondary.Delay = check and AnimDelay or 5-self.Owner:GetUpgradeCount('perma_pimphand')
	end
end
...
function SWEP:PrimaryAttack()
	...
	if timer.Exists("ResetAnim") then
		timer.Remove("ResetAnim")
	end

	timer.Create("ResetAnim", AnimDelay, 1, function()
	...
	if SERVER then
		...
		if tr.Hit then
			...
			if valid and (player or phys) then
				...
				end -- SetVelocity is more practical for players
