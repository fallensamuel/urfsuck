...
if SERVER then
	...
	function SWEP:AttemptBreak()
		...
		if self:GetCuffBroken() >= 65 then
			if IsValid(self:GetOwner()) and self:GetOwner().IsCooldownAction and self:GetOwner():IsCooldownAction("CuffBreaker", 0.2) then 
				return
			end
		end
...
if CLIENT then
	...
	local Text = (translates and {
		translates.Get('Ваши руки %sсвязаны') or 'Ваши руки %sсвязаны',
		translates.Get('крепко') or 'крепко ',
		translates.Get('слабо') or 'слабо ',
		translates.Get('%s материалом') or '%s материалом',
		translates.Get('прочным') or ' прочным',
		translates.Get('тонким') or ' тонким',
		translates.Get('Вам завязали глаза') or 'Вам завязали глаза',
		translates.Get('Вам заткнули рот') or 'Вам заткнули рот',
		translates.Get('[%s] Сопротивляться') or '[%s] Сопротивляться',
		translates.Get('ЛКМ') or 'ЛКМ',
	}) or {
		'Ваши руки %sсвязаны',
		'крепко ',
		'слабо ',
	...
	function SWEP:DrawHUD()
		...
		local TextPos = h+30
		local str = string.format( Text[1],
			((self:GetCuffStrength()>1.2) and Text[2]) or ((self:GetCuffStrength()<0.8) and Text[3]) or ""
		)
		if self:GetCuffRegen()>1.2 or self:GetCuffRegen()<0.8 then
			str = str.. string.format( Text[4], ((self:GetCuffRegen()>1.2) and Text[5]) or Text[6] )
		end
		...
		if self:GetIsBlind() then
			TextPos = TextPos+20
			draw.SimpleText( Text[7], "HandcuffsText", w+1, TextPos+1, Col.TextShadow, TEXT_ALIGN_CENTER )
			draw.SimpleText( Text[7], "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER )
		end
		if self:GetIsGagged() then
			TextPos = TextPos+20
			draw.SimpleText( Text[8], "HandcuffsText", w+1, TextPos+1, Col.TextShadow, TEXT_ALIGN_CENTER )
			draw.SimpleText( Text[8], "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER )
		end
		
		if cvars.Bool( "cuffs_allowbreakout" ) then
			TextPos = TextPos+20
			str = string.format(Text[9], input.LookupBinding("+attack") or Text[10])
			draw.SimpleText( str, "HandcuffsText", w+1, TextPos+1, Col.TextShadow, TEXT_ALIGN_CENTER )
			draw.SimpleText( str, "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER )
		end
