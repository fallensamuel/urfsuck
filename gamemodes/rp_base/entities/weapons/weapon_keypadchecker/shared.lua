...
local function getTargets(keypad, keyPass, keyDenied, delayPass, delayDenied)
	...
	for k, v in pairs(numpad.OnDownItems or {}) do
		...
		if v.key == keyPass and v.ply == Owner then
			table.insert(targets, {
				type = "Entering the right password",
				name = v.name,
				ent = v.ent,
				original = keypad
			})
		...
		if v.key == keyDenied and v.ply == Owner then
			table.insert(targets, {
				type = "Entering a wrong password",
				name = v.name,
				ent = v.ent,
				original = keypad
			})
	...
	for k, v in pairs(numpad.OnUpItems or {}) do
		if v.key == keyPass and v.ply == Owner then
			table.insert(targets, {
				type = "after having entered the right password",
				name = v.name,
				delay = math.Round(delayPass, 2),
				ent = v.ent,
				original = keypad
			})
		end
		...
		if v.key == keyDenied and v.ply == Owner then
			table.insert(targets, {
				type = "after having entered wrong password",
				name = v.name,
				delay = math.Round(delayDenied, 2),
				ent = v.ent,
				original = keypad
			})
		end
...
local function getEntityKeypad(ent)
	...
	for k, v in pairs(ents.FindByClass("urf_keypad")) do
		local vOwner = v:CPPIGetOwner()
		local entOwner = ent:CPPIGetOwner()
...
function SWEP:PrimaryAttack()
	...
	elseif class == "urf_keypad" then
		...
		rp.Notify(self.Owner, NOTIFY_GENERIC, rp.Term('EntityControlledByX'), #data)
	end
...
function numpad.OnDown(ply, key, name, ent, ...)
	...
	table.insert(numpad.OnDownItems, {
		ply = ply,
		key = key,
		name = name,
		ent = ent,
