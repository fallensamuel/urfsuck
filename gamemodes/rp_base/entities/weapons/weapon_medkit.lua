-- "gamemodes\\rp_base\\entities\\weapons\\weapon_medkit.lua"
...
function SWEP:PrimaryAttack()
	...
	if ( IsValid( ent ) && self:Clip1() >= need && ( ent:IsPlayer() or ent:IsNPC() ) && ent:Health() < ent:GetMaxHealth() ) then
		...
		if ent:IsPlayer() and (ent:Health() + need) >= ent:GetMaxHealth() then
			hook.Run( "OnPlayerHealed", self.Owner, ent );
		end
...
hook.Add( "PostGamemodeLoaded", "weapon_medkit", function()
	local weapon_medkit = weapons.GetStored( "weapon_medkit" );

	if not weapon_medkit then
		weapons.Register( SWEP, "weapon_medkit" );
		return
	end

	table.Merge( weapon_medkit, SWEP );
end );