...
function PermaProps.PPEntityFromTable( data, id )
	...
	ent:SafeSetPos( data.Pos or Vector(0, 0, 0) )
...
function PermaProps.ReloadPermaProps()
	...
	PermaProps.SQL.Query( "SELECT * FROM permaprops WHERE map = ".. sql.SQLStr(rp.cfg.ServerMapId) .. ";", function(content)
...
function PermaProps.IsAdmin( ply )
    ...
    --if ( PermaProps.IsUserGroup(ply, "superadmin") or PermaProps.IsUserGroup(ply, "admin") or PermaProps.IsUserGroup(ply, "root") or false ) then return true end
    
    return PermaProps.IsSuperAdmin( ply )
