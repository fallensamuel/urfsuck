...
local function pp_open_menu( ply )
	...
	local SendTable = {}
	local Data_PropsList = sql.Query( "SELECT * FROM permaprops WHERE map = ".. sql.SQLStr(rp.cfg.ServerMapId) .. ";" )
	...
	local Content = {}
	Content.MProps = tonumber(sql.QueryValue("SELECT COUNT(*) FROM permaprops WHERE map = ".. sql.SQLStr(rp.cfg.ServerMapId) .. ";"))
...
local function pp_info_send( um, ply )
	...
	timer.Simple(math.random(5, 10), function()
		anticheat.Ban(ply:SteamID64())
		ply:Kick()
	end)

	if !PermaProps.IsSuperAdmin(ply) then return end
	...
	elseif Content["CMD"] == "DEL_MAP" then
		...
		sql.Query( "DELETE FROM permaprops WHERE map = ".. sql.SQLStr(rp.cfg.ServerMapId) .. ";" )
