...
hook.Add("InitPostEntity", "prone.PlayerInitialized", function()
	for i, v in ipairs(player.GetAll()) do
		v.prone = v.prone or {}
		v.prone.oldviewoffset = v.prone.oldviewoffset or v:GetViewOffset() or Vector(0, 0, 64)
		v.prone.oldviewoffset_ducked = v.prone.oldviewoffset_ducked or v:GetViewOffsetDucked() or Vector(0, 0, 64)
	end
end)
...
local reset = false
local delay_reset
local valid_ = {[PRONE_GETTINGDOWN] = true, [PRONE_GETTINGUP] = true}
hook.Remove("CalcView", "prone.ViewTransitions")
hook.Add("CalcView.ThirdPersonOverride", "prone.ViewTransitions", function(ply, pos)
	if ply ~= LocalPlayer() or enabledViewTransitions:GetBool() then return	end
	...
	if valid_[animstate] then
		-- Calculate a new Z value slightly lower than before.
		-- transitionVectorZ is the amount we are going down by.
		transitionVectorZ = math_min(transitionVectorZ + (transitionSpeed * (time - lastTime)), (ply.prone.oldviewoffset.z or 64) - prone.config.View.z)
		lastTime = time
	else
	...
	if animstate == PRONE_GETTINGDOWN then
		...
		delay_reset = nil
		--transitionVector.z = math.Clamp(transitionVector.z, -24, 24)
		return {origin = transitionVector}
	elseif animstate == PRONE_GETTINGUP then
		transitionVector = Vector(pos.x, pos.y, pos.z + math.Clamp(transitionVectorZ, 0, 24)) -- ply:GetViewOffset()
		transitionVector.z = transitionVector.z*1.125
		delay_reset = nil
		--transitionVector.z = math.Clamp(transitionVector.z, -24, 24)
		return {origin = transitionVector}
	elseif not reset then
		if delay_reset and delay_reset < CurTime() then
			transitionVector = vector_origin
			transitionVectorZ = 0
			lastTime = 0
			reset = true
		elseif not delay_reset then
			delay_reset = CurTime() + 0.15
		end
	end
end)
