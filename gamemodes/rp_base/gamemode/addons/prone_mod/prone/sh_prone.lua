...
function prone.CanEnter(ply)
	if rp.cfg.DisableProneSilent then return false end
	...
	elseif ply:GetMoveType() == MOVETYPE_NOCLIP or not ply:OnGround() or ply:WaterLevel() > 1 then
		return false
	elseif ply:GetEmoteActionSequence() ~= -1 then
		return false
	end
...
function prone.Enter(ply)	
	...
	ply.prone.getdowntime = math.max(length, 1.75) + ply.prone.starttime
...
function prone.End(ply)
	...
	local length = ply:SequenceDuration(ply:LookupSequence(seq))
	ply.prone.getuptime = math.max(length, 1.75) + ply.prone.endtime
...
function prone.Exit(ply, synctocl)
	ply.prone = ply.prone or {}

	ply:ProneViewOffset(ply.prone.oldviewoffset or Vector(0, 0, 64), ply.prone.oldviewoffset_ducked or Vector(0, 0, 28))
	ply:ResetHull()

	ply:SetProneAnimationState(PRONE_NOTINPRONE)
	hook.Call("prone.OnPlayerExitted", nil, ply)

	if SERVER and synctocl then
		net.Start("proneExitsync")
		net.Send(ply)
	end
end

if SERVER then
	util.AddNetworkString("proneExitsync")
else
	net.Receive("proneExitsync", function()
		prone.Exit(LocalPlayer())
	end)
end
...
function prone.Handle(ply)
	...
	if (ply.ProneHandleCooldown or 0) > CurTime() then return end
	ply.ProneHandleCooldown = CurTime() + 3
		...
		if prone.CanExit(ply) then
			...
			if SERVER then
				--timer.Simple(2, function()
					if IsValid(ply) then
						rp.SetMood(ply, ply:GetNetVar("rp.mood"), nil, true)
					end
				--end)
			end
		end
	else
		if SERVER then rp.SetMood(ply, 0, true, true) end
...
local proneMoveSpeed = CreateConVar("prone_movespeed", tostring(prone.config.MoveSpeed), {FCVAR_ARCHIVE, FCVAR_REPLICATED}, "Устанавливает скорость движения в положении лежа.")
...
hook.Add("SetupMove", "prone.Handle", function(ply, cmd, cuc)
	if not rp.cfg.DisableProneSilent then 
		if IsProne(ply) then
...
function PLAYER:ProneViewOffset(vec, vec2)
	local CT = CurTime()
	if (self.LastProneViewOffser or 0) < CT then
		self.LastProneViewOffser = CT + 0.1
		self:SetViewOffset(vec)
		self:SetViewOffsetDucked(vec2 or vec)
	end
end
...
local GetMainActivityAnimation = {
	[PRONE_GETTINGDOWN] = function(ply)
		if GetProneAnimationLength(ply) <= CurTime() then
			ply:ProneViewOffset(prone.config.View or Vector(0, 0, 24))
	...
	[PRONE_INPRONE] = function(ply, velocity)
		...
		if IsValid(weapon) then
			...
			if string.sub(WeaponHoldType, 1, 4) == 'mood' then
				WeaponHoldType = nil
			end
		end
