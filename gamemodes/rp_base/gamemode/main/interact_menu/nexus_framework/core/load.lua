...
function LOADER:GetLoadMessage(realm, path)
	--local col = self:GetLoadColor()
	--local str = "[" .. self:GetAcronym() .. "] "
	
	--MsgC(col, str, color_white, "Loaded ", Color(0, 255, 0), path .. "\n")
end
...
function LOADER:Register()
	...
	--MsgC(self:GetLoadColor(), "[" .. self:GetAcronym() .. "]", color_white, " Finished loading in " .. time .. "\n")
end
