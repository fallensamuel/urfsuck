...
if (SERVER) then
	...
	net.Receive("NetStreamDS", function(length, player)
		...
		if (player.nsDataStreamTS or 0) < ct then
			player.nsDataStreamTS, player.nsDataStreamRecv, player.nsDataStreamRecv_Notify = ct + 10, 0, nil;
		end

		player.nsDataStreamRecv = player.nsDataStreamRecv + 1;
		if player.nsDataStreamRecv > 8 then
			if not player.nsDataStreamRecv_Notify then
				player.nsDataStreamRecv_Notify = true;
				ErrorNoHalt( "NetStream: " .. tostring(player) .. " (" .. player:SteamID() .. ") is exceeding NetStream message rate per tick!\n" );
			end
