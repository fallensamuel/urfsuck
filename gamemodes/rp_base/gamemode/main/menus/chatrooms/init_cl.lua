...
-- 	end,
-- 	/*
-- 	[CHAT_CROSSSERVER] = function(id, msg)
-- 		RunConsoleCommand('say', string.sub('/gc ' .. msg, 1, 123))
-- 	end,
-- 	*/
...
local sendFunctions = {
	[CHAT_LOCAL] = function(id, msg)
		rp.PlayerSay(string.sub(msg, 1, 250))
	end,
	[CHAT_STAFF] = function(id, msg)
		--rp.PlayerSay('@' .. string.sub(msg, 1, 250))
		RunConsoleCommand('say', string.sub('@' .. msg, 1, 250))
	end,
	[CHAT_OOC] = function(id, msg)
		rp.PlayerSay(string.sub('// ' .. msg, 1, 250))
	end,
	[CHAT_PM] = function(id, msg)
		rp.PlayerSay(string.sub('/pm ' .. id .. ' ' .. msg, 1, 250))
	end,
	/*
	[CHAT_CROSSSERVER] = function(id, msg)
		rp.PlayerSay(string.sub('/gc ' .. msg, 1, 250))
	end,
	*/
	[CHAT_GROUP] = function(id, msg)
		rp.PlayerSay(string.sub('/g ' .. msg, 1, 250))
	end,
	[CHAT_ORG] = function(id, msg)
		rp.PlayerSay(string.sub('/org ' .. msg, 1, 250))
	end,
	[CHAT_RADIO] = function(id, msg)
		rp.PlayerSay(string.sub('/radio ' .. msg, 1, 250))
	end
}
...
local function createChatsMenu(switch)
	...
	fr = ui.Create('ui_frame', function(self)
		...
		self:SetTitle(translates.Get('Активные Чаты'))
		self:SetSize(w, h)
		self:Center()
		self.Threads = {}
	...
	fr.newPM = ui.Create('DButton', function(self)
		...
		self:SetText(translates.Get("Новое личное сообщение"))

		self.DoClick = function()
			local m = ui.DermaMenu()
			local list = player.GetAll()
			table.sort(list, function(p1, p2) return p1:Name():lower() < p2:Name():lower() end)

			for k, v in ipairs(list) do
				if (not fr.Threads[v:SteamID()]) then
	...
	fr.sendBtn = ui.Create("DButton", function(self)
		...
		self:SetText(translates.Get("Отправить"))

		self.DoClick = function()
			if (LocalPlayer():GetBVar('ChatMuted')) then 
				fr.textBox:SetText('')
	...
	fr.CreateThread = function(fr, chatType, id, title)
		...
		ui.Create('ui_listview', function(chat)
			...
			btn.DoRightClick = function(s)
				if (not s.Deletable) then return end
				local m = ui.DermaMenu()

				m:AddOption(translates.Get("Удалить обсуждение"), function()
					chat:Remove()
					btn:Remove()
					fr.Threads[id] = nil
	...
	if (LocalPlayer():IsAdmin()) then
		fr:CreateThread(CHAT_STAFF, 'staff', translates.Get('Админ Чат'))
	end

	fr:CreateThread(CHAT_LOCAL, 'local', translates.Get('Локальный'))
	fr:CreateThread(CHAT_OOC, 'ooc', translates.Get('Общий'))
	//fr:CreateThread(CHAT_CROSSSERVER, 'global', translates.Get('Глобальный'))
	fr.Threads["ooc"].btn:DoClick()
end

concommand.Add('rp_chats', function(_, _, a)
	createChatsMenu(a[1])
end)

hook("PlayerUseAdminChat", "ChatRoom.StaffChat", function(pl, msg)
	createChatsMenu('hidden')
	fr:AddMessage(CHAT_STAFF, 'staff', translates.Get('Админ Чат'), pl, msg)
end)
...
hook("ChatRoomMessage", "ChatRoom.ParseMessage", function(chatType, plFrom, plTo, text)
	...
	if (chatType == CHAT_LOCAL) then
		fr:AddMessage(chatType, 'local', translates.Get('Локальный'), plFrom, text)
	elseif (chatType == CHAT_OOC) then
		fr:AddMessage(chatType, 'ooc', translates.Get('Общий'), plFrom, text)
	elseif (chatType == CHAT_PM) then
		...
		/*
	elseif (chatType == CHAT_CROSSSERVER) then
		fr:AddMessage(chatType, 'global', translates.Get('Глобальный'), plFrom, text:sub(3, #text))
		*/
	elseif (chatType == CHAT_GROUP) then
		fr:AddMessage(chatType, 'group', LocalPlayer():GetJobName() .. " Chat", plFrom, text)
	elseif (chatType == CHAT_ORG) and LocalPlayer():GetOrg() then
		fr:AddMessage(chatType, 'org', LocalPlayer():GetOrg() .. " Chat", plFrom, text)
	elseif (chatType == CHAT_RADIO) then
		fr:AddMessage(chatType, 'radio', translates.Get("Радио"), plFrom, text)
	end
end)