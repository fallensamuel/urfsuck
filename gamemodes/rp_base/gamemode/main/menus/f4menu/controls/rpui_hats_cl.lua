...
function PANEL:Init()
    ...
    self.button.Think = function(this)
        ...
        if not ishat and not self.ActiveRenderSWEP then return end
    ...
    self.button.DoClick = function(this)
        ...
        elseif this:GetText() == "Снять" then
            if ishat then
        ...
        elseif this:GetText() == "Купить" or this:GetText() == "" then
            if IsValid(this.currency_select) then this.currency_select:Remove() end
            ...
            link.left.PriceTitle = "За деньги"
            ...
            link.left.PriceText = function()
                ...
                if money >= price then
                    ...
                    return "Не хватает ".. string_Comma(price - money).."$"
                end
            end
            ...
            if self.ThisData and self.ThisData.donatePrice then
                ...
                link.right.PriceTitle = "За донат"
...
function PANEL:CreateToolgunBtn(tooltab)
    ...
    btn.Paint = function(this, w, h)
        ...
        local txt = owned and "Куплено" or ( SwepPrice .. ( tooltab.discount and " (-"..(tooltab.discount*100).."%)" or "" ) ) --this:IsHovered() and (self.active_hats[mdl] and "Экипировать" or "Купить") or rp.FormatMoney(tooltab.price)
...
function PANEL:CreateHatBtn(mdl, data)
    ...
    HatBtn.Paint = function(this, w, h)
        ...
        draw.SimpleText(this["data"].name, "rpui.Fonts.Hats.HatName", h + self.ContentSpacing, h/2, textColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        ...
        local txt = owned and "Куплено" or ( HatPrice .. ( this["data"].discount and " (-"..(this["data"].discount*100).."%)" or "" ) ) --this:IsHovered() and (self.active_hats[mdl] and "Экипировать" or "Купить") or rp.FormatMoney(this["data"].price)
        local tx_w, tx_h = draw.SimpleText(txt, "rpui.Fonts.Hats.HatPrint", w - self.ContentSpacing, h/2, textColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
