...
hook('PlayerShouldTakeDamage', 'AntiPK_PlayerShouldTakeDamage', function(victim, attacker)
	if ( nodamage and nodamage[attacker:GetClass()] ) or ( IsValid(victim) and IsValid(attacker) and victim:IsPlayer() and attacker:IsVehicle() ) then 
		return false
	end
end)
