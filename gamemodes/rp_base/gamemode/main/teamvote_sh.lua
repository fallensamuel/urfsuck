-- "gamemodes\\rp_base\\gamemode\\main\\teamvote_sh.lua"
...
if (SERVER) then
	...
	function rp.teamVote.Create(name, duration, choices, callback)
		...
		local size = #data
		local recipients = table.Filter(player.GetAll(), function(v) return not table.HasValue(choices, v) end)
	...
	function rp.teamVote.Tick()
		...
		for k, v in pairs(rp.teamVote.Votes) do
			...
			if (shouldFinish) then
				...
				for k, v in pairs(v.Votes) do -- TODO: FIX
	...
	net('rp.TeamVote', function(len, pl)
		...
		if (not IsValid(votechoice)) then return end
...
local function createTeamVoteWindow(votename, endtime, options)
	if (IsValid(cdframes[votename])) then
		cdframes[votename]:Remove()
	end

	local fr = ui.Create('ui_frame', function(self)
		self:SetTitle(translates.Get('Выборы %sа', votename))
	...
	local scr = ui.Create('ui_scrollpanel', function(self)
		...
		self.Think = function(self) 
			...
			if (SysTime() > endtime and not self.Closing) then
				self.Closing = true
				fr:Close()
			end
		end
	...
	for k, v in ipairs(options) do
		...
		local btn = ui.Create('DButton', function(self) -- TODO: FIX
			self:SetTall(25)
			self:SetText(name)
			scr:AddItem(self)
	...
	if (LocalPlayer():IsAdmin()) then
		local deny = ui.Create("DButton", function(self)
			...
			self.DoClick = function()
				RunConsoleCommand('urf', 'denyteamvote', votename)
				fr:Close()
			end
...
local function createTeamVoteCountdown(votename, endtime)
	...
	local pnl = ui.Create('panel', function(self)
		...
		self.Paint = function(self, w, h)
			...
			fr:SetTitle(translates.Get('Выборы %sа, через %sс', votename, math.ceil(diff)))
