...
local local_rp_DrawInfoBubble = function(alpha, ico, text, desc, custom_ico_col, ico_rotate, rotate_speed, rotate_offset, ToScreenPos, postDraw, title_colr, no_draw_trigon, InsideScreenPos)
    ...
    if ToScreenPos and InsideScreenPos then
        ...
        posY = math.Clamp(posY, 0, ScrH() - ico_wide*1.25)
    end

    SetDrawColor(ColrAlpha(bubbe_col, alpha))
    if not no_draw_trigon then
        SetMaterial(bubble_mat)
        DrawTexturedRect(posX - ico_wide*0.25, posY + ico_wide, ico_wide*0.5, ico_wide*0.25)
    end
    ...
    if ico_rotate == true then
        ...
        --DrawTexturedRectRotated(_a*0.5, ico_wide*0.5, ico_wide*0.75, ico_wide*0.75, t)
    else
        SetDrawColor(ColrAlpha(custom_ico_col or white_col, alpha))
        if type(ico) == "IMaterial" then SetMaterial(ico) end
        DrawTexturedRect(posX + _a, posY + ico_wide*0.125, ico_wide*0.75, ico_wide*0.75)
    end

    local name_tall = 0
    if is_txt then
        local w, h = SimpleText(text, fonts.title, posX + _a + ico_wide, posY + ico_wide*0.125, ColrAlpha(title_colr or white_col, alpha))
        name_tall = h
    end
    if is_txt2 then
        SimpleText(desc, fonts.desc, posX + _a + ico_wide, posY + ico_wide*0.125 + name_tall - desc_tall*0.125, ColrAlpha(white_col, alpha))
    end
...
rp.DrawInfoBubbleLeft = function(alpha, ico, ToScreenPos, postDraw, drawRight) -- not extended
...
function rp.DrawInfoBubbleRight(alpha, ico, ToScreenPos, postDraw)
    rp_DrawInfoBubbleLeft(alpha, ico, ToScreenPos, postDraw, true)
end
...
hook.Add("HUDPaint", "InitUrfimBubbleHints", function() -- создание таймера только после загрузки игрока
    ...
    local LP, ply_pos, ent_cfg, ent, remain
    ...
    timer.Create("UrfimBubbleHints", 0.5, 0, function()
        if #entsQueue > 0 then return end
...
local drawEntsBubble = function()
    ...
    for ent, data in pairs_(ToDrawTable) do
        ...
        local pos = (ent_cfg.localoffset and ent:LocalToWorld(ent_cfg.localoffset) or ent_pos) + Vec(0, 0, maxs.z)
...
rp.cfg.BubbleHints["CustomENTs"] = function(ent)
    local item_tab = ent.getItemTable and ent:getItemTable()
