-- "gamemodes\\rp_base\\gamemode\\util\\fprofiler\\ui\\_frame_cl.lua"
...
function BOTTLENECKTAB:Init()
    ...
    FProfiler.UI.onCurrentRealmUpdate("bottlenecks", function(new)
        ...
        local recordpath = "fprofiler/records/" .. FProfiler.UI.getModelValue("realm");
        file.CreateDir( recordpath );

        local filename = string.format( "%s/[%s] %s - bottlenecks.txt", recordpath, util.DateStamp(), util.CRC(game.GetIPAddress()) );
        local filecontents = {
            "Address:\t\t"  .. game.GetIPAddress(),
            "Hostname:\t\t" .. GetHostName(),
            "Realm:\t\t\t"  .. FProfiler.UI.getModelValue("realm"),
            "Time:\t\t\t"   .. string.FormattedTime(totalTime or 0, "%02i:%02i:%02i"),
            "",
        };
        ...
        if #filecontents > 5 then
            file.Write( filename, table.concat(filecontents, "\n") );
        end
    end)
...
function TOPTENTAB:Init()
    ...
    FProfiler.UI.onCurrentRealmUpdate("topLagSpikes", function(new)
        ...
        local filename = string.format( "%s/[%s] %s - lagspikes.txt", recordpath, util.DateStamp(), util.CRC(game.GetIPAddress()) );
        local filecontents = {
            "Address:\t\t"  .. game.GetIPAddress(),
            "Hostname:\t\t" .. GetHostName(),
            "Realm:\t\t\t"  .. FProfiler.UI.getModelValue("realm"),
