...
concommand.Add('tps',function(pl)
	local pos = ents.FindByClass('prop_physics')[1]:GetPos()
	pl:SetPos(util.FindEmptyPos(pos))
end)

concommand.Add('tbench',function(pl)
	local pos = ents.FindByClass('prop_physics')[1]:GetPos()

	print(GAMEMODE:FindEmptyPos(pos, {pl}, 600, 30, Vector(16, 16, 64)))
	print(util.FindEmptyPos(pos))

	xbench.Compare({
		Old = function()
			GAMEMODE:FindEmptyPos(pos, {pl}, 600, 30, Vector(16, 16, 64))
		end, 
		New = function()
			util.FindEmptyPos(pos)
		end
	})
end)
