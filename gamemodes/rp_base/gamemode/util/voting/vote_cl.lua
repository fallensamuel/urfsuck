...
local function MsgDoVote(msg)
	...
	panel:SetTitle(translates and translates.Get( "Голосование" ) or "Голосование")
	...
	local time_str = translates and translates.Get( "Время" ) or "Время"
	function panel:Think()
		self:SetTitle(time_str .. ": " .. tostring(math.Clamp(math.ceil(timeleft - (CurTime() - OldTime)), 0, 9999)))
	...
	ybutton:SetCommand("!")
	ybutton:SetText( translates and translates.Get( "Да" ) or "Да" )
	...
	nbutton:SetCommand("!")
	nbutton:SetText( translates and translates.Get( "Нет" ) or "Нет" )
	...
	if LocalPlayer():IsAdmin() then
		...
		nbutton.DoClick = function()
			RunConsoleCommand('urf', 'denyvote', steamid)
			panel:Close()
		end
