...
local function ccDoVote(ply, cmd, args)
	...
	if vote.voters[ply] or vote.exclude[ply] or canVote == false then
		rp.Notify(pl, NOTIFY_ERROR, message or rp.Term('CannotVote'))
...
function GM.vote:create(question, voteType, target, time, callback, excludeVoters, fail, extraInfo)
	excludeVoters = excludeVoters or {
		[target] = true
	}

	local newvote = {}

	setmetatable(newvote, {
		__index = Vote
	})
	...
	if #player.GetAll() <= table.Count(excludeVoters) then
		rp.Notify(target, NOTIFY_GREEN, rp.Term('VoteAlone'))
		newvote:callback(1)
	...
	if target:IsPlayer() then
		rp.Notify(target, NOTIFY_GREEN, rp.Term('VoteStarted'))
	end
	...
	timer.Create(newvote.id .. "DarkRPVote", time, 1, function()
		newvote:handleEnd()
	end)
end
