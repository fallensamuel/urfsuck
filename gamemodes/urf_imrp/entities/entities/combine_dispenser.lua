...
if (CLIENT) then
	...
	function ENT:createRation(activator)
		...
		timer.Simple(1.9, function()
			if (IsValid(self) and IsValid(entity)) then
				...
				rp.item.spawn(ent_ration, pos, function(item, ent)
	...
	function ENT:dispense(amount, activator)
		...
		if (amount < 1 || !IsValid(activator)) or onlyOnePerDay[sid64] then
			self:setUseAllowed(true)
			return 
		end

		onlyOnePerDay[sid64] = true
	...
	function ENT:Use(activator)
		...
		timer.Simple(1, function()
			if IsValid(activator) then
				local amount = activator:GetJobTable().rationCount or 0
