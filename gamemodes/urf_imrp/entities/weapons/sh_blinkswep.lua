...
local MAX_DISTANCE = CreateConVar('sv_blink_distance', '100', {FCVAR_ARCHIVE, FCVAR_REPLICATED}, 'Max distance (in meters)')
...
function SWEP:DrawHUD()
	...
	if not hit then
		...
		draw.DrawText('Дистанция: ' .. dist .. 'м', 'Trebuchet24', x, y, READY)
	end
end
...
local function EntTookDamage(ent, dmginfo)
	if ent:IsPlayer() then
		local awep = ent:GetActiveWeapon()
		if not (IsValid(awep) and awep:GetClass() == 'sh_blinkswep') then return end
		
		awep.NextJump = CurTime() + ATTACK_DELAY:GetFloat()
		awep:NetMessageFinish()
    end
end

hook.Add("EntityTakeDamage", "BlinkSWEP_TookDamage", EntTookDamage)