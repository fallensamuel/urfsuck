...
function ACTION:onCall( ply )
	...
	elseif ply:GetMoveType() == MOVETYPE_LADDER then
		...
		return false
	elseif not ply:IsOnGround() and !IsValid(ply:GetGroundEntity()) then // elseif not ply:IsOnGround() or IsValid(ply:GetGroundEntity()) then
...
if CLIENT then
	function ACTION:onCalcViewFirstperson( ply, origin, angles, FOV )
		...
		local headbone = ply:LookupBone("ValveBiped.Bip01_Head1");
		origin = ply:GetBonePosition( headbone ) or origin;
